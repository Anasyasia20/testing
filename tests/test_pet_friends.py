...
def test_successful_create_pet_info(name='Барбоскин', animal_type='терьер',
                                     age='4'):
...
def test_get_api_key_for_invalid_users(email = valid_email, password = invalid_password)
    status, result = pf.get_api_key(email, password)
    assert status == 200
    assert 'key' in result
